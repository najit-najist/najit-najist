...
export const OrderSubtitle: FC<OrderUnderpageProps> = ({ order, viewType }) => {
  ...
  const {
    payment_method: { delivery_method: deliveryMethod },
  } = order;
    ...
    const orderStateToSubtitle: MessagesDictionary = {
      ...
      shipped:
        typeof deliveryMethod !== 'string' ? (
          isLocalPickup(deliveryMethod) ? (
            <>
              Objednávka byla připravena na prodejně. Čekáme až si pro ni
              uživatel dorazí.
            </>
          ) : (
            <>
              Objednávka byla zabalena a odeslána. Čekáme na informaci od
              dopravce jestli balík dorazil. Pokud dorazil, ale systém tuto
              informaci nezaznamenal tak objednávku přepněte ručně
            </>
          )
        ) : null,
      unpaid: (
        ...
        <>
          {typeof deliveryMethod !== 'string' ? (
            isLocalPickup(deliveryMethod) ? (
              ...
              <>
                Objednávka byla zabalena a předána dopravci. O dalším průběhu
                Vás bude kontaktovat dopravce.
              </>
            )
          ) : null}
        </>
